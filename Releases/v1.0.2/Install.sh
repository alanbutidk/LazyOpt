...
if [ -f "./LazyOpt" ] && [ -f "./server" ]; then
    ...
    if [ "$YesOrNo" = "yes" ]; then
        ...
        curl -L "https://github.com/alanbutidk/LazyOpt/releases/download/unified-v1.0.2/LazyOptLinux64v1.0.2.zip" -o "./LazyOptLinux64v1.0.2.zip"
    else
    ...
    echo "Extracting..."
    unzip "./LazyOptLinux64v1.0.2.zip" -d "$INSTALL_DIR"
    echo "Almost Done! Files extracted to: $INSTALL_DIR"

    echo "Cleaning up..."
    rm "./LazyOptLinux64v1.0.2.zip"

...
if ((Test-Path -Path ".\LazyOpt.exe") -and (Test-Path -Path ".\server.exe")) {
  ...
  if ($YesOrNo -eq "yes") {
    ...
    Invoke-WebRequest -Uri "https://github.com/alanbutidk/LazyOpt/releases/download/win64-v1.0.1/LazyOptWindows64v1.0.1.zip" -OutFile ".\LazyOptWindows64v1.0.1.zip"
  } else {
    Write-Output "No detected! Not downloading the files!"
    exit
  }
  Expand-Archive -Path ".\LazyOptWindows64v1.0.1.zip" -DestinationPath "$extractFolder"
  Write-Output "Almost Done! Files extracted to: $extractFolder"
  Write-Output "Cleaning up..."
  Remove-Item -Path ".\LazyOptWindows64v1.0.1.zip"
